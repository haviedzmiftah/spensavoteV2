  ...
  return (
    <div className="flex h-full w-72 flex-col bg-white shadow-[4px_0_24px_rgba(0,0,0,0.02)] border-r border-gray-100 z-10">
      ...
      <nav className="flex-1 space-y-2 px-4 py-8 overflow-y-auto">
        {navigation.map((item) => (
          <Link
            key={item.name}
            href={item.href}
            className="group flex items-center rounded-2xl px-4 py-3.5 text-sm font-semibold text-gray-500 hover:bg-indigo-50 hover:text-indigo-600 transition-all duration-300"
          >
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gray-50 text-gray-400 group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-colors mr-3 shadow-sm">
      ...
      <div className="p-4 border-t border-gray-100">
        <button className="flex w-full items-center justify-center gap-2 rounded-2xl bg-red-50/80 px-4 py-3.5 text-sm font-bold text-red-600 hover:bg-red-100 transition-colors shadow-sm">
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
