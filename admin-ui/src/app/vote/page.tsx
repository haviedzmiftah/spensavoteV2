    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4 py-12">
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-6 py-4 shadow-sm">
          ...
          <span className="text-sm font-semibold text-slate-700">Memuat kandidat...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-indigo-600">Pemilihan</p>
        ...
        {candidates.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
            <p className="text-lg font-semibold text-slate-700">Belum ada kandidat yang tersedia.</p>
            <p className="mt-2 text-sm text-slate-500">Silakan tunggu hingga data kandidat diunggah oleh panitia.</p>
          </div>
        ) : (
          <div className="grid gap-6 xl:grid-cols-3 md:grid-cols-3">
            {candidates.map((candidate) => (
              <button
                key={candidate.id}
                type="button"
                onClick={() => router.push(`/vote/confirm?candidate=${candidate.id}`)}
              ...
              >
                <div className="relative h-60 bg-gradient-to-br from-indigo-50 via-slate-50 to-purple-100 p-5">
                  {candidate.photoUrl ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white/80 text-center text-slate-500">
                      <div>
                        <div className="text-4xl font-black text-indigo-500">{candidate.candidateNumber}</div>
                        <div className="mt-2 text-xs font-semibold uppercase tracking-[0.2em]">Paslon</div>
                      </div>
                    </div>
                  )}
                ...
                <div className="flex flex-1 flex-col space-y-5 p-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm leading-6 text-slate-600">{candidate.vision}</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <ul className="space-y-2 text-sm text-slate-600">
                      {candidate.mission
                        ...
                        .slice(0, 3)
                        .map((item, index) => (
                          <li key={`${candidate.id}-${index}`} className="flex gap-2 leading-6">
                  ...
                  <div className="mt-auto flex items-center justify-between pt-2">
                    <span className="text-sm font-semibold text-slate-500">Siap untuk dipilih</span>
